  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {success && (
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/40 text-green-700 dark:text-green-400 text-sm rounded-xl px-4 py-3">
            Informações salvas com sucesso!
          </div>
        )}

        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-none p-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="relative w-20 h-20 rounded-2xl overflow-hidden border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-green-400 bg-gray-50 dark:bg-gray-800 flex items-center justify-center transition-colors group shrink-0"
              >
                {logoPreview ? (
                  <>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400 space-y-1">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">PNG, JPG ou WEBP · Máx. 5 MB</p>
                {logoPreview && (
                  <button type="button" onClick={() => setLogoPreview('')} className="text-xs text-red-400 hover:text-red-500 transition-colors">
                    Remover logo
                  </button>
                )}
              </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nome do estabelecimento *</label>
          ...
          <div>
            ...
            <textarea name="description" defaultValue={business.description ?? ''} rows={3} placeholder="Fale um pouco sobre o seu negócio para os clientes..." className={INPUT + ' resize-none'} />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tipo de negócio</label>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-none p-6 space-y-5">
          ...
          <div>
            ...
            <input name="phone" defaultValue={business.phone ?? ''} placeholder="(11) 99999-9999" className={INPUT} />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">E-mail de contato</label>
            <input name="email" type="email" defaultValue={business.email ?? ''} placeholder="contato@seunegocio.com.br" className={INPUT} />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Endereço</label>
            <input name="address" defaultValue={business.address ?? ''} placeholder="Rua Exemplo, 123 — Bairro, Cidade - UF" className={INPUT} />
          </div>
        </div>

        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-none p-6">
          ...
          <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3 border border-gray-200 dark:border-gray-700">
            <span className="text-sm text-gray-500 dark:text-gray-400 flex-1 truncate">/booking/{business.slug}</span>
            <a href={`/booking/${business.slug}`} target="_blank" className="text-sm text-green-600 dark:text-green-400 font-medium hover:text-green-700 dark:hover:text-green-300 shrink-0">
              Abrir →
            </a>
          </div>
