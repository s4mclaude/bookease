...
const STATUS_LABELS: Record<string, { label: string; className: string }> = {
  pending:   { label: 'Pendente',   className: 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400' },
  confirmed: { label: 'Confirmado', className: 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' },
  completed: { label: 'Concluído',  className: 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' },
  canceled:  { label: 'Cancelado',  className: 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400' },
}
    ...
    <div className="p-4 md:p-8 max-w-3xl">
      ...
      {customers.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-12 text-center">
          <p className="text-gray-400 dark:text-gray-500 text-sm">Nenhum cliente ainda.</p>
          <p className="text-gray-300 dark:text-gray-600 text-xs mt-1">Os clientes aparecem aqui após o primeiro agendamento.</p>
        </div>
      ) : (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-none overflow-hidden">
          {customers.map((c, i) => {
            ...
            return (
              <div key={c.id} className={!isLast ? 'border-b border-gray-100 dark:border-gray-800' : ''}>
                <button
                  onClick={() => toggle(c.id)}
                ...
                >
                  ...
                  <div className="text-right shrink-0 mr-2">
                    ...
                    {c.last_visit && (
                      <p className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500 mt-0.5 justify-end">
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 px-5 py-3">
                    {clientAppts.length === 0 ? (
                      <p className="text-xs text-gray-400 dark:text-gray-500 py-2">Nenhum agendamento registrado.</p>
                    ) : (
                      <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                        {clientAppts.map((a) => {
                          const st = STATUS_LABELS[a.status] ?? { label: a.status, className: 'bg-gray-100 dark:bg-gray-800 text-gray-500' }
                          return (
                            <div key={a.id} className="flex items-center gap-3 py-2.5">
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{a.service_name}</p>
                    ...
                    <a
                      ...
                      target="_blank"
                      className="inline-flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400 font-medium hover:text-green-700 dark:hover:text-green-300 transition-colors mt-3"
                    >
                      <Phone className="w-3 h-3" />
